    ...
    <>
      <div className="flex min-h-screen flex-col justify-evenly px-4 md:justify-between [@media(min-width:600px)_and_(max-height:540px)]:pt-6">
        ...
        >
          ...
          <p className="mx-auto mt-4 w-full px-4 text-[1rem] leading-normal tracking-normal text-pretty text-foreground xs:mt-2 xs:max-w-lg xs:text-[1.2rem] sm:max-w-lg sm:px-2 sm:text-[1.35rem] sm:leading-[1.4] md:mt-0 md:px-0 md:text-[1.4rem] lg:text-[1.45rem] xl:max-w-3xl xl:px-12 xl:text-[1.5rem] xl:leading-[1.3] 2xl:px-2 2xl:text-[1.55rem] 2xl:leading-[1.35] [@media(min-width:600px)_and_(max-height:540px)]:text-[1.1rem]">
            {farewellHeroData.body} The song{" "}
            <span className="px-px text-primary">{farewellHeroData.song}</span>{" "}
            by{" "}
            <span className="pl-px text-black dark:text-white">
